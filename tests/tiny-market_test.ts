...
function mintFt({ chain, deployer, amount, recipient, paymentAssetContract = defaultPaymentAssetContract}: 
    { chain: Chain, deployer: Account, amount: number, recipient: Account, paymentAssetContract?: string}) {
...
Clarinet.test({
    ...
    async fn(chain: Chain, accounts: Map<string, Account>) {
        ...
        const { nftAssetContract, tokenId } = mintNft({ chain, deployer, recipient: maker });
        const { paymentAssetContract } = mintFt({ chain, deployer, recipient: taker, amount: price });
        const order: Order = { tokenId, expiry: 10, price: price + 10, paymentAssetContract };
